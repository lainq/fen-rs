...
struct fen fen_for_test(enum fen_player player, char* ep, int wk, int wq,
                        int bk, int bq) {
  ...
  if (ep == NULL) {
    ...
    fobj.enpassent =
        (struct fen_enpassent_target){.rank = ep[1] - '0', .file = ep[0]};
  }
...
int compare_fen_properties(const struct fen* curr, const struct fen* cmp) {
  int result =
      COMPARE_CASTLING_ABILITY(curr, cmp, black) &&
...
#define GENERATE_TEST_CASE(ntn, player, enp, c1, c2, c3, c4)           \
  (struct test_case) {                                                 \
    .notation = ntn, .data = fen_for_test(player, enp, c1, c2, c3, c4) \
  }

#define PLAYER_STRING(player) player == FEN_PLAYER_WHITE ? "white" : "black"
#define PRINT_ENPASSENT(fobj)                                    \
  if (fobj->is_enpassent_target_none) {                          \
    printf("None ");                                             \
  } else {                                                       \
    printf("%c%u ", fobj->enpassent.file, fobj->enpassent.rank); \
  }
...
void fen_debug_print(const struct fen* a, const struct fen* b) {
  printf("Player:%s | %s\n", PLAYER_STRING(a->player),
         PLAYER_STRING(b->player));
  printf("Enpassent: ");
  PRINT_ENPASSENT(a);
  PRINT_ENPASSENT(b);
  printf("\n");
...
int main() {
  const struct test_case test_cases[test_cases_c] = {
      ...
      GENERATE_TEST_CASE("8/8/8/8/8/8/8/8 w Qk - 0 1", FEN_PLAYER_WHITE, NULL,
                         0, 1, 1, 0)};
  struct fen fobj;
  size_t success = 0;
  char* cback = NULL;
